...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      currentMouse[0] = (currentMouse[0] ?? 0.5) + 0.05 * (tx! - (currentMouse[0] ?? 0.5));
      currentMouse[1] = (currentMouse[1] ?? 0.5) + 0.05 * (ty! - (currentMouse[1] ?? 0.5));
  ...
  return <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }} className={className} />;
};
